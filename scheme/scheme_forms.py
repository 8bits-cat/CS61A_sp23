...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_expr = do_lambda_form(Pair(formals, body), env)
        env.define(symbol, lambda_expr)
        return symbol
        #lambda_expr = Pair(Pair('lambda', Pair(formals, body)), nil)
        #define_expr = Pair(symbol, lambda_expr)
        #return do_define_form(define_expr, env)
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        vals = Pair(eval_all(clause.rest, env), vals)
