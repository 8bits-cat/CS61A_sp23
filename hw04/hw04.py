...
def balanced(m):
    ...
    if is_planet(m):
        ...
        l, r = left(m), right(m)
        return length(l) * total_weight(end(l)) == length(r) * total_weight(end(r)) and balanced(end(l)) and balanced(end(r))
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        return tree(total_weight(m),[totals_tree(end(f(m))) for f in [left, right]])
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        return True in [has_path(b, word[1:]) for b in branches(t) if label(t) == word[0]]
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0 or upper_bound(y) < 0, "divisor can't use spans zero interval"
