...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_list = [diff_function(typed_word, elem, limit) for elem in word_list]
        index, min_diff= [], min(diff_list)
...
def feline_fixes(typed, source, limit):
    ...
    if typed == '' or source == '':
        ...
        if typed[0] == source[0]:
            return feline_fixes(typed[1:], source[1:], limit) + 0
        else:
...
def final_diff(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        add = final_diff(typed, source[1:], limit - 1) + 1
        remove = final_diff(typed[1:], source, limit - 1) + 1
        substitute = final_diff(typed[1:], source[1:], limit - 1) + 1
        # BEGIN
        return min(add, remove, substitute)
        # END
...
def time_per_word(words, times_per_player):
    ...
    for elem in times_per_player:
        times += [[ elem[i] - elem[i - 1] for i in range(1, len(elem))]]
...
def fastest_words(match):
    ...
    for i in word_indices:
        cost_per_player += [[time(match, j, i) for j in player_indices]]
        #for each word all players cost times
    for i in player_indices:
        word_fast += [[]]   #creat [] for each player
    for i in word_indices:
        fast = cost_per_player[i][0] # init the fasted player and the time
        who = 0
        for j in player_indices:
            if cost_per_player[i][j] < fast: # the time ith word jth player cost 
                fast = cost_per_player[i][j]
                who = j
        word_fast[who] += [get_word(match, i)] # found the fasted player, add ith word to his list
    return word_fast
    # END PROBLEM 10
