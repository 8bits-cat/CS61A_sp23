...
def tail_points(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    assert opponent_score >= 0, 'opponent_score must be non-negative'
...
# BEGIN PROBLEM 4
perfect_square = lambda x: x % sqrt(x) == 0
next_perfect_square = lambda x: int((sqrt(x) + 1) * (sqrt(x) + 1))
# END PROBLEM 4
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def simple(score, opponent_score):
        return n
    return simple
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    def total_call(*args):
        result, loop = 0, 0
        while loop < total_samples:
            result , loop = result + original_function(*args), loop + 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_rolls in range(1,11):
       curr = averaged_dice(num_rolls, dice)
       if curr > max_score:
           max_score = curr
           r_num_rolls = num_rolls
    return r_num_rolls
...
def final_strategy(score, opponent_score):
    ...
    2.Use tail and square strategy, but instead of using a threshold,
    roll 0 whenever it would give you more points on average than rolling 6.
    """
    # BEGIN PROBLEM 12
    num_rolls = 6
    threshold = make_averaged(roll_dice, 100)(num_rolls)
    any_strategy_suc = not(square_strategy(score, opponent_score, threshold) and tail_strategy(score, opponent_score,threshold))
    if any_strategy_suc:
        num_rolls = 0
    else:
        if score > opponent_score:
