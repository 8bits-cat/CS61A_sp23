    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.is_container:
                container, other = self, place.ant
            else:
                container, other = place.ant, self
            place.ant = container
            container.store_ant(other)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        self.attack_place = self.place
        self.current_place = self.lower_bound
        for _ in range(self.lower_bound):
                if self.attack_place.entrance:
                    self.attack_place = self.attack_place.entrance
        while self.lower_bound <= self.current_place <= self.upper_bound:
            if self.attack_place.bees:
                if not self.attack_place.is_hive:
                    return random_bee(self.attack_place.bees)
                else:
                    return None
            if self.attack_place.entrance:
                self.attack_place = self.attack_place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        targets = list(self.place.bees)
        super().reduce_health(amount)
        for target in targets:
            if self.health == 0:
                target.reduce_health(amount + self.damage)
            else:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt have a large health value, but do nothing"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt eating bee need 3 turns to chew"""
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3
    implemented = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem 13
        if target is not None:
            target.slowing_turns = 5

            def slow_action(gamestate):
                if target.slowing_turns > 0:
                    target.slowing_turns -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        current_place, distance = self.place, 0
        all_insects_in_front = {}
        while current_place is not None:
            if current_place.ant:
                if current_place.ant is not self:
                    all_insects_in_front[current_place.ant] = distance
            if current_place.bees:
                if not current_place.is_hive:
                    for bee in current_place.bees:
                        all_insects_in_front[bee] = distance
            current_place = current_place.entrance
            distance = distance + 1
        return all_insects_in_front
        # END Problem Optional 2
